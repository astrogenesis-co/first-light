...
test('body discoveries include transit unlocks without exposing locked entries', () => {
  ...
  assert.deepEqual(bodyDiscoveryKeys('planet-7', before), ['memories/first-glimpse', 'transmissions/approaching-planet-7'])
  assert.deepEqual(bodyDiscoveryKeys('planet-7', during), ['memories/first-glimpse', 'songs/mock-signal', 'transmissions/approaching-planet-7'])
  const arrived = unlockedCodexKeys(scenario('planet-7-orbit'))
  assert.equal(bodyDiscoveryKeys('planet-7', arrived).length, 6)
